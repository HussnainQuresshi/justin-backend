...
router.post("/booking", async (req, res) => {
  ...
  try {
    const { processId } = req.body;
    const process = await Process.findById(processId);
    const userName = process.passengerName;
...
router.post("/driver-arrived", async (req, res) => {
  ...
  try {
    ...
    console.log("Booking message error", err);
    return res.status(400).send({
      message: "driver failed to arrive",
    });
  }
  ...
  return twilioClient.messages
    .create({
      body:
        ...
        " with " +
        NoOfPassengers +
        " passenger/s going to " +
        destination +
        " whose destination is " +
        destination +
        " please ask the passenger to end the trip when you arrive to the destination ",
      from: process.env.TWILIO_PHONE_NUMBER,
      to: drivercpnum,
    })
    .then((res) => {
